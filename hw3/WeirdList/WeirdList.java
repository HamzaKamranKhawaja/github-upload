...
public class WeirdList {
    ...
    /** A new WeirdList whose head is TIP and tail is END. */
    public WeirdList(int Tip, WeirdList End) {
        this.Tip = Tip;
        if(End == null) {
            this.End = EMPTY;
        } else {
    ...
    /** Returns number of elements in the sequence that
     *  starts with THIS. */
    public int length() {
        Length += 1;
        return End.length();
    ...
    public String toString() {
        return " " + String.valueOf(Tip) + End.toString();
    }

    /** Part 3b: Apply FUNC.apply to every element of THIS WeirdList in
     *  sequence, and return a WeirdList of the resulting values. */
    public WeirdList map(IntUnaryFunction func) {
        return new WeirdList(func.apply(this.Tip), this.End.map(func));
    }
