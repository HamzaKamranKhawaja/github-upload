...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
        ...
        value = presentValue * Math.pow(1 + rate/100 , years);
        return value;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double interestValue = CompoundInterest.futureValue(presentValue, rate, targetYear);
        double realDollarValue =  CompoundInterest.futureValue( interestValue, - inflationRate, targetYear );



        return realDollarValue;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        for(int i = 0; i <= years; i++ ){
            sum = sum + futureValueReal(perYear, rate, targetYear-i, 0);
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        int years = numYears(targetYear);
        for(int i = 0; i < years; i++ ){
            sum = sum + futureValueReal(perYear, rate, targetYear-i, inflationRate);
        }
    ...
    static void printDollarFV(int targetYear, double returnRate,
                              double inflationRate) {
        double nominalDollarValue = CompoundInterest.futureValue(1.00, returnRate, targetYear);
        double realDollarValue =  CompoundInterest.futureValueReal(1.00, returnRate, targetYear, inflationRate );
    ...
    static void printSavingsFV(int targetYear, double returnRate,
        ...
        double nominalSavings = CompoundInterest.totalSavings(perYear, targetYear, returnRate);
        double realSavings = CompoundInterest.totalSavingsReal(perYear, targetYear, returnRate, inflationRate);
